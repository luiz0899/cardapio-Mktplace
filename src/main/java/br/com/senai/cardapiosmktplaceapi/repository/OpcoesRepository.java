...
public interface OpcoesRepository extends JpaRepository<Opcao,Integer> {
	...
	@Query(value = 
			...
			+ "WHERE o.id = :id " )
	public Opcao buscarPor(@Param("id") Integer id);
	
	@Modifying
	@Query(value = "UPDATE Opcao o SET o.status = :status WHERE o.id = :id ")
	public Opcao atualizarPor(Integer id, Status status);
	...
	@Query("UPDATE Opcao o " 
	       ...
	       + "WHERE o.id = :id")
	public void atualizarOpcao(@Param("id") Integer id, @Param("nome") String nome,
