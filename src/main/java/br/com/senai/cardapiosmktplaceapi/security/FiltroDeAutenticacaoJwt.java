...
public class FiltroDeAutenticacaoJwt extends OncePerRequestFilter {
	...
	@Autowired
	private GerenciadorDeTokenJwt gerenciadorDeTokenJwt;
	...
	protected void doFilterInternal(HttpServletRequest request, 
		...
		if (isHeaderAuthorizationPresente) {
			...
			login = gerenciadorDeTokenJwt.extrairLoginDo(token);
		}
		...
		if (isNovoLogin) {
			...
			if (gerenciadorDeTokenJwt.isValidado(token, credencial)) {
				
				UsernamePasswordAuthenticationToken tokenAutenticado =
						new UsernamePasswordAuthenticationToken(
								credencial, null ,credencial.getAuthorities());
				tokenAutenticado.setDetails(new WebAuthenticationDetailsSource()
